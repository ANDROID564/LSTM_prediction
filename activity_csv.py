...
dataset = read_csv('walking.csv',names=colnames)
#dataset = read_csv('walking1.csv',header=0, index_col=0)
print(dataset)
...
#print all the values in a csv file

myFile = open('Predicted_Activity.csv', 'w')  
with myFile:    
    #writer = csv.writer(myFile)
    myFields = ['Predicted_Activity'] 
    
    writer = csv.DictWriter(myFile, fieldnames=myFields, lineterminator='\n')    
    writer.writeheader()  #used for writing the header file
    for i in range(len(inv_y12)):
        writer.writerow({'Predicted_Activity': inv_y12[i]})
